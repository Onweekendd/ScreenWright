!(function (undefined) {
  'use strict';
  var _global,
    colorFormat = function (options) {
      return new _colorFormat(options);
    },
    _colorFormat = function (options) {
      var result,
        ...
        hslType = -1 == format.indexOf('hsla') ? 0 : 1;
      if (color.indexOf('#') > -1)
        'hex' == format
          ? ((result = this.hexToRgb(color)), (result = this.rgbToHex(result)))
          : format.indexOf('rgb') > -1
          ? (result = this.hexToRgb(color, rgbType))
          : format.indexOf('hsl') > -1 && ((result = this.hexToRgb(color)), (result = this.rgbToHsl(result, hslType)));
      else if (color.indexOf('rgb') > -1)
        (result = this.getRgb(color, rgbType)),
          'hex' == format
            ? (result = this.rgbToHex(result))
            : format.indexOf('hsl') > -1 && (result = this.rgbToHsl(result, hslType));
      else if (color.indexOf('hsl') > -1)
        (result = this.getHsl(color, hslType)),
          (result = this.hslToRgb(result, rgbType)),
          'hex' == format
            ? (result = this.rgbToHex(result))
            : format.indexOf('hsl') > -1 && (result = this.rgbToHsl(result, hslType));
      else {
        ...
        resultRgb && resultRgb.length > 0
          ? 'hex' == format
            ? ((result = this.hexToRgb(resultRgb)), (result = this.rgbToHex(result)))
            : format.indexOf('rgb') > -1
            ? (result = this.hexToRgb(resultRgb, rgbType))
            : format.indexOf('hsl') > -1 &&
              ((result = this.hexToRgb(resultRgb)), (result = this.rgbToHsl(result, hslType)))
          : alert('参数color暂未定义');
      }
      return result;
    };
  (_colorFormat.prototype = {
    ...
    getRgb: function (rgb, type) {
      var flag = -1 == (rgb = rgb.toLowerCase()).indexOf('rgba') ? 0 : 1;
      rgb = (rgb = flag ? rgb.replace('rgba', '') : rgb.replace('rgb', '')).replace(/\s/g, '').split(',');
    ...
    getHsl: function (hsl, type) {
      var flag = -1 == (hsl = hsl.toLowerCase()).indexOf('hsla') ? 0 : 1;
      hsl = (hsl = flag ? hsl.replace('hsla', '') : hsl.replace('hsl', '')).replace(/\s/g, '').split(',');
    ...
    rgbToHex: function (rgb) {
      ...
      return (
        ...
        red[0] == red[1] && green[0] == green[1] && blue[0] == blue[1] && opacity[0] == opacity[1] && (simpleType = 1),
        {
          ...
          o: opacity,
          complete: simpleType
            ? '#' + red[0] + green[0] + blue[0] + (1 == rgb.o ? '' : opacity)
    ...
    rgbToHsl: function (rgb, type) {
      ...
      if (max == min) h = s = 0;
        ...
        var d = max - min;
        switch (((s = l < 0.5 ? d / (max + min) : d / (2 - max - min)), max)) {
          case r:
            h = (g - b) / d;
            break;
          case g:
    ...
    hexToRgb: function (hex, type) {
      var red,
        ...
        blue,
        opacity,
        hexsplit = (hex = hex.replace('#', '')).split('');
      return (
        3 == hex.length
    ...
    hslToRgb: function (hsl, type) {
      ...
      if (0 == s) r = g = b = l;
        ...
        (r = hue2rgb(temp1, temp2, (h /= 360) + 1 / 3)),
          (g = hue2rgb(temp1, temp2, h)),
          (b = hue2rgb(temp1, temp2, h - 1 / 3));
      }
      return {
        r: (r = Math.round(255 * r)),
        g: (g = Math.round(255 * g)),
        b: (b = Math.round(255 * b)),
        o: o,
