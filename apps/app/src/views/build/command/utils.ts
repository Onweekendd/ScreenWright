...
export const createDebouncedBatchUpdater = <T extends { id: string | number }>(
  ...
  const addUpdate = (updateItem: T) => {
    ...
    debouncedFlush();
    console.log("添加到待处理更新", {
      componentId,
      pendingUpdatesCount: pendingUpdates.size,
      action: pendingUpdates.has(componentId) ? "覆盖" : "新增"
    });
  };
...
export const debugForceLRUCleanup = async () => {
  try {
    // 私有方法，这里只是为了演示
    console.log("注意：这是一个调试函数，实际使用时需要通过正常的数据操作触发LRU清理");
    ...
    console.log("已添加测试数据，这可能会触发LRU清理机制");
  } catch (error) {
