  ...
  >
    <template #default>
      <el-form-item label="判断类型" :label-width="85">
        <sw-radio direction="row" :option="judgeType" v-model="currentEvent.conditionType" />
      </el-form-item>
      ...
      <div class="condition-list">
        <configCustom
          ...
          v-for="(item, index) in currentEvent.conditions"
          :key="item.id"
          :is-action="true"
...
import configCustom from "./configCustom.vue";
import configSelect from "./configSelect.vue";
import { useConditionConfig } from "./useConditionConfig";
...
} = useConditionConfig(currentEvent);
