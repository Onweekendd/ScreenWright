...
abstract class BaseChart implements ComponentInstanceType {
  ...
  createTooltip(
    ...
    return {
      ...
      backgroundColor: option.tooltipBackground ? "rgba(0, 0, 0, 0)" : "rgba(0, 0, 0, 0.8)",
      borderWidth: 0,
      padding: [
      ...
      position: (point: any) => {
        return [point[0] + (option.tooltipOffsetX || 0), point[1] + (option.tooltipOffsetY || 0)];
      },
      axisPointer: {
        lineStyle: {
      ...
      formatter: (params: any) => {
        ...
        str += `<div style="padding-top: ${
          option.tooltipLabelGapSapce >= 0 ? option.tooltipLabelGapSapce : screenScale * 20
        }px">`;
  ...
  // 处理 系类 line bar 数据
  getEchartsAxisNameAndSeriesData(optionData: any[]): { axisName: string[]; optionData: any[] } {
    // 获取所有的 axisName
    let axisName: any[] = [];
  ...
  setPieSeriesData(option: optionsProps) {
    option.legendSeriesWidthType = isUndefined(option.legendSeriesWidthType) ? "auto" : option.legendSeriesWidthType;

    option.legendSeriesWidth = isUndefined(option.legendSeriesWidth) ? 100 : option.legendSeriesWidth;
  }
