...
class Echartbar extends BaseChart {
  ...
  markLineLabelFontWeight: Array<"normal" | "bold" | "bolder" | "lighter" | number> = [];
    ...
    this.option.dataSeriesName.forEach((item: any, index: number) => {
      ...
      if (idx !== -1) {
        ...
        this.seriesFieldList.forEach((field: any) => {
          (this[field as keyof Echartbar] as any[])[idx] = this.option[field][index];
        });
        ...
        const list = (optionsData || []).map((item: any, index: number) => {
          return {
            ...
            itemStyle: {
              ...
              barBorderRadius: (this.option.barBorderRadius === "default" ? 0 : [50, 50, 0, 0]) || 0
            },
            label: {
                ...
                ? [
                    {
                      ...
                      yAxis:
                        item.markLineData ||
                        (this.markLineDataType[index] === "custom" ? this.option.markLineData[index] : null)
                    }
                  ]
                : []
            },
