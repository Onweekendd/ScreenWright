  ...
  <div class="layout-item">
    ...
    >
      <div>
          ...
          <template v-if="componentType === 'customTableList' && currentEvent.trigger === EventTypeEnum.Click">
            <el-form-item label="按钮对象" :label-width="85">
              <el-select v-model="currentEvent.btnObjs" popper-class="sw-select-dropdown" @change="update">
                <el-option v-for="item in btnObjsOptions" :key="item.value" :label="item.label" :value="item.value" />
              </el-select>
            </el-form-item>
          </template>
        ...
        <condition-config ref="conditionConfigRef" type="events" :current-event-index="currentIndex" />
      </div>
    </config-custom>
  </div>
</template>
...
import { customActionTypeOptions } from "@/views/build/components/buildConfig/constants";
...
import ComponentActionConfigs from "./ComponentActionConfigs.vue";
...
const addEvent = async () => {
  const obj = ["threeScene", "industryScene"].includes(selectTargetData.value[0].component.prop as string)
...
const addAction = () => {
  ...
  let temp = templateActions();
  if (currentEvent.value.actions && currentEvent.value.actions.length) {
    temp = cloneDeep({
      ...currentEvent.value.actions[currentEvent.value.actions.length - 1],
      id: "action_" + uuid()
    }) as Action;
  }
  currentEvent.value.actions.push(temp);
  handleActionTabs(temp.id);
  update();
};

const deleteAction = () => {
  if (!currentAction.value?.id || !currentEvent.value) return;
