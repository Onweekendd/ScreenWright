      ...
      <template #item="{ element, index }: { element: PanelState; index: number }">
        ...
        >
          ...
          <div
            ...
            @dblclick="($event) => handleDblStatus({ ...element, index }, $event)"
...
const { activeStatusId, panelData, panelInfo } = usePanelInfo();
const { addPanelStatus, changePanelStatus, onStatusOrderChange } = usePanelAction();
...
const handleDblStatus = (item: StatusItem, event: MouseEvent): void => {
...
const handleSave = (index: number) => {
  ...
  if (statusActiveName.value.trim()) {
    ...
    updateComponentLayers(panelInfo.value.config as DynamicPanelProps);
  }
};
