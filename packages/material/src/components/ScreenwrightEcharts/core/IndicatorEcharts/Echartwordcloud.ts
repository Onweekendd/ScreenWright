...
class Echartwordcloud extends BaseChart {
      ...
      series: [
        {
          ...
          maskImage: this.validData(this.option.maskImageShow, true)
            ? maskImage
            : undefined,
          ...
          // 尺寸范围
          sizeRange: [
            this.option.sizeRangeMin || 0,
            this.option.sizeRangeMax || 0,
          ],

          // 旋转范围
          rotationRange: [
            this.option.rotationRangeMin || 0,
            this.option.rotationRangeMax || 0,
          ],
          // 旋转步长
          ...
          textStyle: {
            ...
            color: () => {
              ...
              if (
                ...
                this.option.seriesColor.length > 0
              ) {
                const index = Math.floor(
                  Math.random() * this.option.seriesColor.length,
                );
                colors =
                  this.option.seriesColor[index] || this.option.seriesColor[0];
              }
              return colors;
            },
          },
  ...
  async init(baseChartProps: BaseChartProps) {
    ...
    if (this.option.maskImageShow && this.option.maskImage) {
      maskImage = await this.loadImage(
        setMinioUrl(this.option.maskImage) || "",
      );
