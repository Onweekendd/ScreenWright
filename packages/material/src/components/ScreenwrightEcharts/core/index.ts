import { AbstractComponent } from "@screenwright/core";
...
export const getBaseEchartInstanceToType = (
  instanceType: Partial<typeof echartInstanceToType>,
) => {
  const result = [];
  for (const key in instanceType) {
    const InstanceClass =
      instanceType[key as keyof typeof echartInstanceToType];
    if (InstanceClass) {
