  ...
  getPie3D(pieData: any[], internalDiameterRatio: number) {
    ...
    legendData.forEach((item, index) => {
      ...
      option.legend.textStyle.rich[`value_${index}`] = {
        ...
        color:
          (this.option.legendValueColorFollow
            ? this.option.seriesColor[index]
            : this.option.legendValueColor) || "rgba(255, 255, 255, 1)",
        fontStyle: this.option.legendValueFontStyle || "normal",
        fontWeight: this.option.legendValueFontWeight || "normal",
        width:
          this.option?.legendSeriesWidthType === "custom"
            ? this.option?.legendSeriesWidth
            : "auto",
      };
  ...
  async getChartData() {
    ...
    this.seriesFieldList.forEach((field) => {
      if (this.option[field]) {
        (this[field as keyof EchartthreePie] as any) = [];
      }
    });
    this.option.dataSeriesName.forEach((item: any, index: number) => {
      ...
      if (idx !== -1) {
        ...
        this.seriesFieldList.forEach((field) => {
          (this[field as keyof EchartthreePie] as any[])[idx] =
