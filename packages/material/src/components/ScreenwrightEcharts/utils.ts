...
function calculateGradientCoordinate(
  ...
  if (
    angle < alpha || // 处于第一象限
    (angle > 180 - alpha && angle < 180) || // 处理第二象限
    (angle > 180 && angle < 180 + alpha) || // 处理第三象限
    angle > 360 - alpha // 处理第四象限
  ) {
  ...
  // 当渐变轴与矩形的交点落在垂直线上
  if (
    (angle > alpha && angle < 90) || // 处理第一象限
    (angle > 90 && angle < 90 + alpha) || // 处理第二象限
    (angle > 180 + alpha && angle < 270) || // 处理第三象限
    (angle > 270 && angle < 360 - alpha) // 处理第四象限
  ) {
    // 将角度乘以（PI/180）即可转换为弧度
  ...
  if (totalLength > lineLength) {
    for (let i = 0; i < Math.ceil(totalLength / lineLength); i++) {
      ...
      result +=
        ...
        (i < Math.ceil(totalLength / lineLength) - 1 ? "\n" : "");
    }
  } else {
    result = data;
  }
 ...
 */
function getMaxColorListByData(
  data: any[],
  commonColor: any,
  hoverColor: any,
  isSort: boolean,
  ...
  return map(data, (item: { name: any }, index: number) => {
    return includes(top3Names, item.name)
      ? isSort
        ? getTop3LinearColor[index]
        : hoverColor
      : commonColor;
  });
}
...
function getColorListByData(data: any[], direction: string) {
  ...
  return map(data, (item, index) => {
    ...
    const filteredColors = Object.fromEntries(
      Object.entries(colorConfig.colors).filter(
  ...
  return {
    ...
    formatter: (params: { value: string }[]) => {
      ...
      str += `<p style="color:#fff;font-size:16px;text-align:center;"><span style="font-family:'Source Han Sans CN-Normal, Source Han Sans CN';font-size:28px;">${
        fixedNum
          ? Number(params[0].value).toFixed(fixedNum)
  ...
  return {
    ...
    rich: {
      ...
      line: {
        ...
        backgroundColor: {
          image:
            "data:image/png;base64,",
        },
      },
    },
  };
...
export function getEchartsColor(color: any) {
