...
export default function useIotFormSlider(element: ComponentType) {
  ...
  const containerStyle = computed(() => {
    ...
    return {
      paddingTop: `${option.value.paddingTop || 0}px`,
      paddingBottom: `${option.value.paddingTop || 0}px`,
  ...
  const activeColorValue = computed(() => {
    ...
    if (
      ...
      // 占比 取整
      console.log(dataChart.value[0], "dataChart.value[0].valu");
  ...
  const pointColorValue = computed(() => {
    if (option.value.pointImage && option.value.pointType === "image") {
      const images = option.value.pointImage;
  ...
  const borderRadius = computed(
    () =>
      `${isUndefined(option.value.borderRadius) ? 3 : option.value.borderRadius}px`,
  );
