...
export const useSwiper = (options: ComponentType) => {
  ...
  const getTextBackgroundStyle = (item: ImageItem): CSSProperties => {
    if (item.textShowBackground && item.textBackground) {
      return {
        display: "flex",
        justifyContent: "center",
        width: `${item.textWidth ? item.textWidth : 100}%`,
        padding: `${item.textPaddingTop || 2}px ${item.textPaddingRight || 2}px ${item.textPaddingBottom || 2}px ${item.textPaddingLeft || 2}px`,
        boxSizing: "border-box",
        minHeight: `${item.fontSize || 12}px`,
        whiteSpace: "normal",
        wordBreak: "break-word",
        overflowWrap: "anywhere",
        "background-color": "transparent",
        "background-image": `url(${setMinioUrl(item.textBackground)})`,
