  ...
  <div class="page-query-config">
    ...
    <SwCollapseItem title="背景" open>
      <template #content>
        ...
        >
          <SwSingleColorPicker v-model="selectTargetData[0].option.hoverBackgroundColor" @change="update" />
        </el-form-item>

        <template v-if="selectTargetData[0].option.hoverBackgroundType == 'custom'">
          <el-form-item label="类型" :label-width="secondLabelWidth">
            <el-select
              popper-class="sw-select-dropdown"
              v-model="selectTargetData[0].option.hoverBackgroundImageType"
              @change="update"
    ...
    <SwCollapseItem title="描边" open>
      <template #content>
        <el-form-item label="颜色" :label-width="secondLabelWidth">
          <SwSingleColorPicker v-model="selectTargetData[0].option.hoverBorderColor" @change="update" />
        </el-form-item>
        <el-form-item label="粗细" :label-width="secondLabelWidth">
          <SwInputNumber unit="px" v-model="selectTargetData[0].option.hoverBorderWidth" @change="update" />
        </el-form-item>
      </template>
    </SwCollapseItem>
  </div>
