...
const checkValidate = (): Promise<boolean> =>
  new Promise((resolve) => {
    ...
    validateForm.value.validate((valid) => resolve(valid));
  });
...
const splitByTopLevelComma = (value: string): string[] => {
  ...
  for (let i = 0; i < value.length; i++) {
    ...
    if (char === ")") { depth -= 1; if (depth < 0) return []; continue; }
    if (char === "," && depth === 0) { result.push(value.slice(start, i).trim()); start = i + 1; }
  }
  if (depth !== 0) return [];
...
const validate = async () => {
  ...
  if (res) {
    ...
    return { name: inputValue, success: res };
  }
  return { success: false, name: "" };
};
