<template>
  <div class="ring-indicator3d-sign">
    ...
    <sw-collapse-item open title="数字系列" v-model="currentDataObj.showNum" showIcon @change="update">
      <template #icon>
        <Icon type="CirclePlus" style="color: #fff; margin-bottom: 10px; font-size: 15px" @click="handleAddNumSeries" />
        <Icon type="Delete" style="color: #fff; margin-bottom: 10px; font-size: 15px" @click="handleDeleteNumSeries" />
      </template>
      <template #content>
        <ScreenwrightSeriesTabs v-model="seriesNumTabs" :tabs="tapNumList" @change="handleNumChange" />
        <div v-if="currentNumData !== null">
          <el-form-item label="映射名" :label-width="secondLabelWidth">
            <sw-input v-model="currentNumData.Mapping" :controls="false" @change="update" />
          </el-form-item>
          <el-form-item label="文本样式" :label-width="secondLabelWidth">
            <configTextStyle v-model="numberFontInput" @change="handleNumberChange" />
            <div class="flex">
              <sw-input-number
                v-model.number="currentNumData.numLetterSpacing"
                unit="px"
                bottomLabel="字距"
                :controls="false"
                @change="update"
              />
              <sw-input-number
                v-model.number="currentNumData.numLineHeight"
                unit="px"
                bottomLabel="行距"
                :controls="false"
                @change="update"
          ...
          <el-form-item label="间距" :label-width="secondLabelWidth">
            <div class="flex flex-center-between">
              <sw-input-number
                v-model.number="currentNumData.numOffsetLeft"
                unit="px"
                bottomLabel="X"
                :controls="false"
                @change="update"
              />
              <sw-input-number
                v-model.number="currentNumData.numOffsetTop"
                unit="px"
                bottomLabel="Y"
                :controls="false"
                @change="update"
          ...
          <sw-collapse-item title="后缀" open v-model="currentDataObj.showNumUnit" showIcon @change="update">
            <template #content>
              <el-form-item label="内容" :label-width="thirdLabelWidth">
                <sw-input v-model="currentNumData.numUnitText" :controls="false" @change="update" />
              </el-form-item>
              <el-form-item label="文本样式" :label-width="thirdLabelWidth">
                <configTextStyle v-model="unitFontInput" @change="handleUnitChange" />
                <div class="flex">
                  <sw-input-number
                    v-model.number="currentNumData.numUnitLetterSpacing"
                    unit="px"
                    bottomLabel="字距"
                    :controls="false"
                    @change="update"
                  />
                  <sw-input-number
                    v-model.number="currentNumData.numUnitLineHeight"
                    unit="px"
                    bottomLabel="行距"
                    :controls="false"
                    @change="update"
              ...
              <el-form-item label="偏移" :label-width="thirdLabelWidth">
                <div class="flex flex-center-between">
                  <sw-input-number
                    v-model.number="currentNumData.numUnitOffsetLeft"
                    unit="px"
                    bottomLabel="X"
                    :controls="false"
                    @change="update"
                  />
                  <sw-input-number
                    v-model.number="currentNumData.numUnitOffsetTop"
                    unit="px"
                    bottomLabel="Y"
                    :controls="false"
                    @change="update"
...
const seriesNumTabs = ref<string>("数字1");
// numList
const tapNumList = computed(() => {
  return selectTargetData.value[0].option.placard[placardType.value].numList
    ? selectTargetData.value[0].option.placard[placardType.value].numList.map((item: any) => item.tabsName)
    : [];
});

const handleAddNumSeries = () => {
  const index = selectTargetData.value[0].option.placard[placardType.value].numList.findIndex((item: any) => {
    return item.tabsName === seriesNumTabs.value;
  });
  const obj = cloneDeep(selectTargetData.value[0].option.placard[placardType.value].numList[index]);
  obj.tabsName = "数字" + (selectTargetData.value[0].option.placard[placardType.value].numList.length + 1);
  obj.content = "";
  selectTargetData.value[0].option.placard[placardType.value].numList.push(obj);

  seriesNumTabs.value = "数字" + selectTargetData.value[0].option.placard[placardType.value].numList.length;
  getCurrent();
  update();
};

const handleDeleteNumSeries = () => {
  if (selectTargetData.value[0].option.placard[placardType.value].numList.length === 1) {
    return;
  }
  const index = selectTargetData.value[0].option.placard[placardType.value].numList.findIndex(
    (it: any) => it.tabsName === seriesNumTabs.value
  );
  selectTargetData.value[0].option.placard[placardType.value].numList.splice(index, 1);

  selectTargetData.value[0].option.placard[placardType.value].numList.forEach((item: any, index: number) => {
    item.tabsName = "数字" + (index + 1);
  });
...
const currentNumData = ref<any | null>(null);
const getCurrent = () => {
  currentData.value = selectTargetData.value[0].option.placard[placardType.value].iconList.find(
    (it: any) => it.tabsName === seriesTabs.value
  );

  currentNumData.value = selectTargetData.value[0].option.placard[placardType.value].numList.find(
    (it: any) => it.tabsName === seriesNumTabs.value
  );

  console.log("getCurrent", currentData.value, currentData.value, currentNumData.value);
...
watch(
  ...
  () => {
    ...
    seriesNumTabs.value = "数字1";
    seriesTabs.value = "图标1";
    getCurrent();
    nameInit("placard." + pathAttrs);
  }
);

const currentNumStatusAttrs = computed(() => {
  const index = selectTargetData.value[0].option.placard[placardType.value].numList.findIndex((item: any) => {
    return item.tabsName === seriesNumTabs.value;
  });
  return `placard.${placardType.value}.numList[${index}]`;
});
...
const {
  ...
  color: "numColor",
  attrs: currentNumStatusAttrs.value
});
...
watch(
  () => currentNumStatusAttrs.value,
  (newValue) => {
    if (newValue) {
      numberInit(currentNumStatusAttrs.value);
      unitInit(currentNumStatusAttrs.value);
    }
  }
);
const {
