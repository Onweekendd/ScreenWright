 ...
 * @example
 * ```typescript
 * const data: echartareaLineData = [
 *   { seriesName: "系列一", name: "A", value: 2024 },
 *   { seriesName: "系列二", name: "A", value: 1378 }
 * ];
 * ```
 */

// 单个数据项的 Schema
const echartareaLineDataItemSchema = z.object({
  name: z.string(),
  seriesName: z.string(),
  value: z.number()
});
...
// 数据数组 Schema（最终导出的类型）
export const echartareaLineDataSchema = z.array(echartareaLineDataItemSchema);

export type echartareaLineData = z.infer<typeof echartareaLineDataSchema>;
 ...
 * 面积折线图配置选项 Schema
 */
export const echartareaLineOptionSchema = z.object({
  // ============ 基础配置 ============
  ...
  // ============ 面积配置 ============
  seriesAreaColor: z.array(gradientColorSchema).describe("面积颜色配置（渐变）"),
  seriesAreaOpacity: z.array(z.number()).describe("面积透明度（0-100）"),

  // ============ 线条配置 ============
  seriesLineColor: z.array(z.string()).describe("线条颜色"),
  seriesLineWidth: z.array(z.number()).describe("线条宽度"),
  seriesSmoothShow: z.array(z.boolean()).describe("是否启用曲线（开启后才生效 seriesSmooth）"),
  seriesSmooth: z.array(z.number()).describe("曲线张力（0-1）：值越大曲线越直，值越小曲线越圆滑"),
  seriesConnectNulls: z.array(z.boolean()).describe("是否连接空值"),
  seriesSymbolShow: z.array(z.boolean()).describe("是否显示标记"),
  seriesSymbol: z.array(z.string()).describe("标记类型（circle/rect/diamond等）"),
...
export type echartareaLineOption = z.infer<typeof echartareaLineOptionSchema>;
