...
// 单个数据项的 Schema
const echartlineDataItemSchema = z.object({
  name: z.string(),
  seriesName: z.string(),
  value: z.number()
});
...
// 数据数组 Schema（最终导出的类型）
export const echartlineDataSchema = z.array(echartlineDataItemSchema);

export type echartlineData = z.infer<typeof echartlineDataSchema>;

// 渐变颜色配置
const gradientColorSchema = z
  .object({
 ...
 * 折线图配置选项 Schema
 */
export const echartlineOptionSchema = z.object({
  // ============ 基础配置 ============
  ...
  shadowBlur: z.array(z.number()).optional().describe("阴影模糊度"),
  shadowColor: z.array(z.string()).optional().describe("阴影颜色"),
  tooltipLabelGapSapce: z.number().optional().describe("Tooltip 标签间距")
});

export type echartlineOption = z.infer<typeof echartlineOptionSchema>;
