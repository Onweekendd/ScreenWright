 ...
 * @example
 * ```typescript
 * const data: echartloopRingPieData = [
...
// 单个数据项的 Schema
const echartloopRingPieDataItemSchema = z.object({
  seriesName: z.string(),
  value: z.number()
});

// 数据数组 Schema（最终导出的类型）
export const echartloopRingPieDataSchema = z.array(echartloopRingPieDataItemSchema);

export type echartloopRingPieData = z.infer<typeof echartloopRingPieDataSchema>;
 ...
 * 轮播环形饼图配置选项 Schema
 */
export const echartloopRingPieOptionSchema = z.object({
  // ============ 基础配置 ============
  ...
  seriesLabelValueBottomPadding: z.number().describe("标签数值底部内边距"),
  ...
  piePaddingNum: z.number().describe("饼图扇区间隔数量"),
  ...
  // ============ 系列强调效果 ============
  seriesEmphasisScale: z.boolean().describe("是否启用系列强调效果"),
  seriesEmphasisScaleSize: z.number().describe("系列强调效果大小")
});

export type echartloopRingPieOption = z.infer<typeof echartloopRingPieOptionSchema>;
