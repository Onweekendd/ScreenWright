...
// 单个数据项的 Schema
const echartthreePieDataItemSchema = z.object({
  seriesName: z.string(),
  value: z.number()
});

// 数据数组 Schema（最终导出的类型）
export const echartthreePieDataSchema = z.array(echartthreePieDataItemSchema);

export type echartthreePieData = z.infer<typeof echartthreePieDataSchema>;
 ...
 * 3D饼图配置选项 Schema
 */
export const echartthreePieOptionSchema = z.object({
  // ============ 基础配置 ============
  ...
  legendOrder: z.enum(["default", "ascending", "descending", "desc"]).describe("图例排序方式"),
  ...
  // ============ 3D配置 ============
  size: z.number().describe("3D饼图大小比例"),
  distance: z.number().describe("3D观察距离"),
  boxHeight: z.number().describe("3D饼图厚度"),
  autoRotate: z.boolean().describe("是否自动旋转"),
  projection: z.enum(["orthographic", "perspective"]).describe("投影方式"),
  autoRotateSpeed: z.number().describe("自动旋转速度"),
  viewControlX: z.number().describe("视角控制X轴角度"),
  viewControlY: z.number().describe("视角控制Y轴角度"),
  viewControlZ: z.number().describe("视角控制Z轴角度"),
  alpha: z.number().describe("3D视角alpha角度"),
  beta: z.number().describe("3D视角beta角度")
});

export type echartthreePieOption = z.infer<typeof echartthreePieOptionSchema>;
