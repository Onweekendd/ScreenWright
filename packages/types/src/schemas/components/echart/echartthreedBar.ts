 ...
 * @example
 * ```typescript
 * const data: echartthreedBarData = [
 *   { name: "A", value: 2024 },
 *   { name: "B", value: 1423 }
 * ];
 * ```
...
// 单个数据项的 Schema（name 兼容数字类目，如年份）
const echartthreedBarDataItemSchema = z.object({
  name: z.union([z.string(), z.number()]),
  value: z.number()
});

// 数据数组 Schema（最终导出的类型）
export const echartthreedBarDataSchema = z.array(echartthreedBarDataItemSchema);

export type echartthreedBarData = z.infer<typeof echartthreedBarDataSchema>;
 ...
 * 3D柱状图配置选项 Schema
 */
export const echartthreedBarOptionSchema = z.object({
  // ============ 基础配置 ============
  ...
  seriesColor: z
    .union([gradientColorSchema, z.array(gradientColorSchema)])
    .describe("系列颜色配置（渐变，单个对象或数组）"),
  seriesTopColor: z.string().describe("系列头部颜色"),
  seriesBottomColor: z.string().describe("系列底部颜色"),
  hoverColor: z
    .union([gradientColorSchema, z.array(gradientColorSchema)])
    .describe("悬停颜色配置（渐变，单个对象或数组）"),
...
export type echartthreedBarOption = z.infer<typeof echartthreedBarOptionSchema>;
