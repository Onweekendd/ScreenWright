...
function resetTextareaSize() {
  ...
  if (!/^\s*$/.test(inputText)) {
    ...
    const dealText = typeof inputText === 'number' ? inputText + '' : inputText;
    const txts = dealText.split ? dealText.split('\n') : dealText.split;
    const maxTxtSize = Math.max(...txts.map((it) => it.length));
    const tlOffset = textlineEl.offset();
    const fontWidth = tlOffset.width / dealText.length;
...
export default class Editor {
  ...
  setText(text) {
    ...
    setText.call(this, text, typeof text === 'number' ? text.toString().length : text.length);
    resetTextareaSize.call(this);
  }
}
