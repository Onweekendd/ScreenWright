...
import { bind, unbind, mouseMoveUp, bindTouch, createEventEmitter } from './event';
...
function selectorSet(multiple, ri, ci, indexesUpdated = true, moving = false) {
  ...
  // 20231122-特殊调整
  const modeType = ri === -1 ? 'col' : ci === -1 ? 'row' : 'range';
  contextMenu.setMode(modeType);
...
function verticalScrollbarSet() {
  ...
  const labelHeight = data.rows.height || 0;
  // console.log('erth:', erth, data.rows.totalHeight());
  verticalScrollbar.set(height, data.rows.totalHeight() - erth - labelHeight);
}

function horizontalScrollbarSet() {
  const { data, horizontalScrollbar } = this;
  const { width } = this.getTableOffset();
  const labelWidth = data.cols.width || 0;
  if (data) {
    horizontalScrollbar.set(width, data.cols.totalWidth() - labelWidth);
  }
}
...
function sheetInitEvents() {
  ...
  bind(window, 'click', onClick);
  bind(window, 'paste', onPaste);
  bind(window, 'keydown', onKeyDown);

  const unbindEvents = () => {
    unbind(window, 'resize', onResize);
    unbind(window, 'click', onClick);
    unbind(window, 'paste', onPaste);
...
export default class Sheet {
  constructor(targetEl, data) {
    ...
    this.unbindEvents = sheetInitEvents.call(this);
    sheetReset.call(this);
    // init selector [0, 0]
