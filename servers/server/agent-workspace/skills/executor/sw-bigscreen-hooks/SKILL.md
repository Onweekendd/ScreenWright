...
description: >
  FunBI 大屏 SDK 核心 Hooks 全局入口与参考指南。
  ...
  **不管回调参数与事件的机制**：cbArgs / callbackArgs / listenArgs / openFilter 的语义与门禁
  已在系统提示词里讲完，源码级依据在 sw-data-flow 的 references/callback-chain.md，不要来这里找。
---

# FunBI 大屏 SDK Hooks 全局入口

SDK 导出文件：`apps/funBI/src/views/view/exportEntry/sdk/hooks.ts`
