...
[
  ...
  { "seriesName": "2025年", "name": "1月", "value": 1900 }
]
```
...
基于配置文件：`echartlineGlobal.vue` / `echartlineSeries.vue` / `echartlinexAxis.vue` / `echartsTooltip.vue`
...
| `seriesSmooth` | boolean[] | 是否使用平滑曲线 |
| `seriesSmoothShow` | boolean[] | 平滑曲线显示开关 |
...
| `seriesAreaColor` | GradientColor[] | 面积填充颜色（仅面积折线图） |
| `seriesAreaOpacity` | number[] | 面积不透明度（仅面积折线图） |
...
```json
{
  "seriesSmooth": [true, true],
  "seriesLineWidth": [2, 2],
  "seriesSymbolShow": [true, true],
  "seriesColor": [
    {"colors": [{"color": "#3e43f4"}]},
    {"colors": [{"color": "#3de3fb"}]}
  ]
...
```json
{
  "seriesAreaColor": [{"colors": [{"color": "#3e43f4", "offset": 0}]}],
  "seriesAreaOpacity": [0.3]
}
```
