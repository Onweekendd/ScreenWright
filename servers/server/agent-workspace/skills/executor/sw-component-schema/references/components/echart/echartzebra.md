# echartzebra (斑马柱状图) 配置说明

## dataChart 数据格式

```typescript
interface ZebraBarDataItem {
  name: string;        // 分类名称
  value: number;       // 数值
  [key: string]: any;
}

type dataChart = ZebraBarDataItem[];
```
...
基于配置文件：`echartzebraGlobal.vue` / `echartzebraxAxis.vue` / `echartzebraTooltip.vue`

斑马柱状图使用 `echartthinBarOptions` 配置选项：**全局、坐标轴、提示框**
