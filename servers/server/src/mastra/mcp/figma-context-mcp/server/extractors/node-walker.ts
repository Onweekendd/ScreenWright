...
import { isVisible } from "@/mastra/mcp/figma-context-mcp/server/utils/common";
import { hasValue } from "@/mastra/mcp/figma-context-mcp/server/utils/identity";

import type { ExtractorFn, GlobalVars, SimplifiedNode, TraversalContext, TraversalOptions } from "./types";
...
export function extractFromDesign(
  ...
  if (nodes.length !== 1) {
    throw new Error("Only one node is allowed");
  }
