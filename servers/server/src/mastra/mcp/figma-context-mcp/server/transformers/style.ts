import type { Node as FigmaDocumentNode, Paint, RGBA, Transform, Vector } from "@figma/rest-api-spec";

import { generateCSSShorthand, isVisible } from "@/mastra/mcp/figma-context-mcp/server/utils/common";
import { hasValue, isStrokeWeights } from "@/mastra/mcp/figma-context-mcp/server/utils/identity";
 ...
 */
function parsePatternPaint(raw: Extract<Paint, { type: "PATTERN" }>): Extract<SimplifiedFill, { type: "PATTERN" }> {
  /**
...
export function convertColor(color: RGBA, opacity = 1): ColorValue {
  ...
  // 使用 Number.isFinite 兜底，避免上游 NaN（?? 只能拦 null/undefined）穿透到 CSS
  const colorAlpha = Number.isFinite(color.a) ? (color.a as number) : 1;
  const validOpacity = Number.isFinite(opacity) ? opacity : 1;
  const a = Math.round(validOpacity * colorAlpha * 100) / 100;
...
function findExtendedLineIntersections(start: Vector, end: Vector): number[] {
  ...
  const uniqueIntersections = [...new Set(intersections.map((t) => Math.round(t * 1000000) / 1000000))];
...
function mapDiamondGradient(
  ...
  _edge: Vector,
  _widthHandle: Vector,
  _findLineIntersectionselementBounds: { width: number; height: number }
): { stops: string; cssGeometry: string } {
  const centerX = Math.round(center.x * 100);
  const centerY = Math.round(center.y * 100);
