import type { HasFramePropertiesTrait, HasLayoutTrait, Rectangle, StrokeWeights } from "@figma/rest-api-spec";

import type { CSSHexColor, CSSRGBAColor } from "@/mastra/mcp/figma-context-mcp/server/transformers/style";
...
/**
 * 检查值是否为 truthy（非空、非 undefined、非 null、非 false、非 0）
 * 用作类型守卫函数
 */
export function isTruthy<T>(val: T): val is NonNullable<T> & (T extends boolean ? true : T) {
  return !!val;
}
...
export function isRectangle<T, K extends string>(key: K, obj: T): obj is T & { [P in K]: Rectangle } {
